...
export class CanvasLocal {
 ...
 this.graphics.fillRect(this.iX(x), this.iY(y)-celdatamaño, celdatamaño,celdatamaño);
  ...
  paint() {
    function textoABinario(texto: string): string {
    ...
    if(url){
     ...
     // console.log("binario enlace:", datosQR);
      ...
      const tamanoQR: number = 25;
      let matrizQR: number[][] = new Array(tamanoQR).fill(null).map(() => new Array(tamanoQR).fill(0));
      //inserta datos en la matriz
      function insertarDatosQR(matriz: number[][], datos: string) {
        let index = 0;
